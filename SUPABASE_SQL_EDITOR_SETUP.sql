...
-- ---------------------------------------------------------------------------
-- Migration 11: 0011_particle-compatibility-profiles.sql
...
VALUES
  ...
  ('41f366d6c21ffaae29875f0b2f1368feb2c9117437eb22ef3b990b6bb94824b6', 1784373600000);
...
REVOKE INSERT, UPDATE, DELETE, TRUNCATE, REFERENCES, TRIGGER
  ON TABLE public."chain_transactions", public."indexed_blocks", public."indexer_cursors", public."canonical_logs", public."reorg_incidents", public."receipts", public."particle_compatibility_profiles", public."particle_profile_release_bindings" FROM opentab_runtime;
...
-- UUID-based.
GRANT SELECT ON TABLE public."user_identities", public."merchants", public."products", public."signed_order_intents", public."orders", public."payment_attempts", public."provider_operations", public."canonical_logs", public."receipts", public."wallet_accounts", public."delegation_records", public."bootstrap_grants", public."live_acceptance_evidence", public."particle_compatibility_profiles", public."particle_profile_release_bindings" TO opentab_evidence_writer;
...
BEGIN
  ...
  SELECT EXISTS (
    ...
    WHERE namespace.nspname = 'public'
      AND relation.relkind IN ('r', 'p')
      AND relation.relname = ANY (ARRAY['chain_transactions', 'indexed_blocks', 'indexer_cursors', 'canonical_logs', 'reorg_incidents', 'receipts', 'particle_compatibility_profiles', 'particle_profile_release_bindings']::text[])
      AND (
  ...
  required_read := ARRAY['user_identities', 'merchants', 'products', 'signed_order_intents', 'orders', 'payment_attempts', 'provider_operations', 'canonical_logs', 'receipts', 'wallet_accounts', 'delegation_records', 'bootstrap_grants', 'live_acceptance_evidence', 'particle_compatibility_profiles', 'particle_profile_release_bindings']::text[];
  SELECT EXISTS (
    SELECT 1
    FROM pg_catalog.pg_class relation
    INNER JOIN pg_catalog.pg_namespace namespace ON namespace.oid = relation.relnamespace
